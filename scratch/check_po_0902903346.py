...
try:
    ...
    SELECT erp.CUSTOMERREF, erp.[Material Code], erp.PRODUCT,
        ...
        CASE WHEN COALESCE(c_exact.[Số TK], c_fallback.[Số TK]) IS NULL THEN 'Missing Declaration' ELSE NULL END as computedMissing
    ...
    print(f"\nFetched {len(rows)} rows for PO 0902903346:")
    cols = [column[0] for column in cursor.description]
    for r in rows:
        d = dict(zip(cols, r))
        print("  MatCode:", d['Material Code'], "| DeclNo:", d['computedDeclarationNumber'], "| Country:", d['computedCountryRegion'], "| isMainFabric:", d['isMainFabric'], "| computedMissing:", d['computedMissing'])
