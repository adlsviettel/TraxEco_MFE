...
y as varchar)+'+'+ItemGroup
				...
				LEFT JOIN [192.168.43.20].[AXDB].[dbo].[VENDTABLE] c ON a.Configuration = c.ACCOUNTNUM COLLATE Chinese_Taiwan_Stroke_BIN

				LEFT JOIN [192.168.43.20].[AXDB].[dbo].[DIRPARTYTABLE] d on c.PARTY = d.RECID
...
Qty,PlanID
		...
		ELSE IF @type = 104
			...
			BEGIN
				...
				LEFT JOIN (SELECT a.[ACCOUNTNUM],b.[NAME] Name
				  FROM [192.168.43.20].[AXDB].[dbo].[VENDTABLE] as a LEFT JOIN [192.168.43.20].[AXDB].[dbo].[DIRPARTYTABLE] as b on a.PARTY=b.RECID) c ON b.Configuration COLLATE SQL_Latin1_General_CP1_CI_AS = c.ACCOU
NTNUM
