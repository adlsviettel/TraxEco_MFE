...
export const exportRdItemsToExcel = async (
  ...
  if (type === 'PRODUCT') {
    columns = [
      ...
      { header: t('rdMaterial.special_feature', 'Special Feature'), key: 'description', width: 30 },
      { header: t('rdMaterial.technology', 'Technology'), key: 'technology', width: 20 },
  ...
  items.forEach((item) => {
    ...
    if (type === 'PRODUCT') {
      rowData = {
        ...
        technology: item.product?.technology || '',
