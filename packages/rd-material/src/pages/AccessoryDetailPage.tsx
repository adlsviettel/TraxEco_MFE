...
import {
  ...
  Menu, MenuItem, ListItemIcon, ListItemText, ClickAwayListener
} from '@mui/material';
...
import AccessoryFormDrawer from './AccessoryFormDrawer';
...
const InfoRow = ({ label, value, italic }: { label: string; value?: React.ReactNode; italic?: boolean }) => (
  <Box>
    ...
    <Typography component="div" sx={{ fontSize: 16, color: value ? '#111827' : '#6b7280', fontWeight: value ? 600 : 500, fontStyle: italic ? 'italic' : 'normal', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
      {value ?? '—'}
    </Typography>
  </Box>
);
...
const AccessoryDetailPage: React.FC = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      // No children needed for accessories
      ...
      try {
        ...
        console.error('[AccessoryDetail] Scan logs failed:', logErr);
        setScanLogs([]);
      }
    } catch {
  ...
  return (
    <Box sx={{ 
      ...
      m: { xs: -1, md: -1.5 }, 
      p: { xs: 2, md: 4 },
      pb: { xs: 'calc(100px + env(safe-area-inset-bottom))', md: 4 },
      display: 'flex',
      ...
      <Box sx={{ flexShrink: 0 }} display="flex" alignItems="center" gap={1} mb={3}>
        <Button size="small" onClick={() => navigate(`${BASE}/accessory`)} sx={{ color: '#6b7280', fontWeight: 500, fontSize: 14, textTransform: 'none', minWidth: 0, p: 0, '&:hover': { color: '#111827', bgcolor: 'transparent' } }}>
          <ArrowBackIcon sx={{ fontSize: 16, mr: 1 }} /> Accessory Hanger
        </Button>
      ...
      <Box sx={{ flexShrink: 0, position: 'relative', pr: { xs: 5, md: 8 } }} mb={4}>
        <Box display="flex" alignItems="center" gap={1.5} mb={1.5} flexWrap="nowrap" sx={{ minWidth: 0 }}>
          ...
          <Chip label="Accessory" size="small" sx={{ bgcolor: '#dbeafe', color: '#1e40af', fontWeight: 600, fontSize: 12, height: 24, textTransform: 'uppercase', letterSpacing: '0.05em', flexShrink: 0 }} />
          
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 1.5, py: 0.5, borderRadius: 2, bgcolor: '#f0fdf4', border: '1px solid #dbeafe', flexShrink: 0, ml: 'auto' }}>
             <Typography fontSize={14} lineHeight={1}>📦</Typography>
             <Typography fontSize={12} fontWeight={700} color="#1e40af">Stock:</Typography>
        ...
        <Typography fontSize={14} color="#4b5563" display="flex" alignItems="center" gap={1.5} flexWrap="wrap">
          ...
          <Box component="span"><Box component="span" fontWeight={600}>QR:</Box> AC-{item.id}</Box>
        </Typography>
          ...
          <Box sx={{ flex: 1, overflowY: { xs: 'visible', md: 'auto' }, pr: { xs: 0, md: 1 }, pb: { xs: 2, md: 0 } }}>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              <Card elevation={0} sx={{ borderRadius: 4, p: 3, bgcolor: 'background.paper', border: '1px solid #f3f4f6', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05), 0 2px 4px -1px rgba(0, 0, 0, 0.03)' }}>
                <Typography fontSize={12} fontWeight={700} color="#9ca3af" textTransform="uppercase" letterSpacing="0.05em" mb={2}>{t('rdMaterial.accessory_images', 'Accessory Images')}</Typography>
                <ImageGallery images={rdItemApi.hasImage(item.mainImage) ? item.mainImage!.split(',').filter(Boolean) : (rdItemApi.hasImage(item.stickerImage) ? item.stickerImage!.split(',').filter(Boolean) : [])} />
              </Card>
              {rdItemApi.hasImage(item.mainImage) && rdItemApi.hasImage(item.stickerImage) && (
                <Card elevation={0} sx={{ borderRadius: 4, p: 3, bgcolor: 'background.paper', border: '1px solid #f3f4f6', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05), 0 2px 4px -1px rgba(0, 0, 0, 0.03)' }}>
          ...
          <Box sx={{ flex: 1, overflowY: { xs: 'visible', md: 'auto' }, pr: { xs: 0, md: 1 } }}>
            ...
            <Card elevation={0} sx={{ borderRadius: 4, bgcolor: 'background.paper', border: '1px solid #f3f4f6', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05), 0 2px 4px -1px rgba(0, 0, 0, 0.03)', overflow: 'hidden' }}>
              <Tabs value={tab} onChange={(_, v) => setTab(v)}
                ...
                <Tab label={t('rdMaterial.general', 'General')} />
              ...
              <Box sx={{ p: 4 }}>
                {tab === 0 && (
                  <Box display="grid" gridTemplateColumns={{ xs: 'repeat(2, 1fr)', sm: 'repeat(3, 1fr)', lg: 'repeat(4, 1fr)' }} columnGap={{ xs: 2, md: 4 }} rowGap={{ xs: 3, md: 5 }}>
                    ...
                    <InfoRow label="Surcharge" value={[item.moqSurcharge ? `MOQ: ${item.moqSurcharge}` : '', item.mcqSurcharge ? `MCQ: ${item.mcqSurcharge}` : ''].filter(Boolean).join(' | ') || undefined} />
                    <InfoRow label="Leadtime" value={item.leadTime} />
                    <InfoRow 
                      label="Quantity" 
                      value={item.quantity !== undefined && item.quantity !== null ? `${item.quantity} ${item.quantityUnit || item.priceUnit || ''}`.trim() : '–'} 
                    />
                                ...
                                {filteredLogs.slice(scanPage * scanRowsPerPage, scanPage * scanRowsPerPage + scanRowsPerPage).map((log, idx) => (
                                  <TableRow key={idx} hover>
                                    ...
                                    <TableCell sx={{ textAlign: 'center', width: 100 }}>
                                      <Chip label={log.actionType} size="small" sx={{ bgcolor: log.actionType === 'IN' ? '#dbeafe' : '#fee2e2', color: log.actionType === 'IN' ? '#1e40af' : '#dc2626', fontWeight: 600, fontSize: 11, height: 20 }} />
                                    </TableCell>
