...
import {
  ...
  Divider, Stack, Snackbar, Alert, CircularProgress, InputAdornment, Autocomplete, Collapse, Checkbox, FormControlLabel, Grid, Tooltip
} from '@mui/material';
...
const AccessoryFormDrawer: React.FC<Props> = ({ open, item, isCopy, onClose, onSaved }) => {
  const navigate = useNavigate();
  const { t } = useTranslation();
  const isEdit = !!item && !isCopy;

  const [form, setForm] = useState<Partial<Item & { cuttableWidth: number }>>({});
  ...
  const specificationRef = React.useRef<HTMLInputElement | null>(null);
    ...
    if (open && !optsLoaded.current) {
      const loadOptions = async (field: string, setter: React.Dispatch<React.SetStateAction<string[]>>) => {
        try {
          ...
          console.error(`[AccessoryFormDrawer] Failed to load options for ${field}:`, err);
        }
      };
  ...
  const handleSave = async (printAfter = false) => {
    ...
    const isMissingSpec = !(form as any).specification;
    const qty = toNum(form.quantity);
    const isMissingQty = qty === undefined || qty <= 0;
    const hasPrice = form.price !== undefined && form.price !== null && form.price !== '';
    const isMissingCurrency = hasPrice && !form.currency;
    const isMissingPriceUnit = hasPrice && !form.priceUnit;

    if (isMissingCode || isMissingSpec || isMissingQty || isMissingCurrency || isMissingPriceUnit) {
      const newErrors = {
        itemCode: isMissingCode,
        specification: isMissingSpec,
      ...
      } else if (isMissingSpec) {
        specificationRef.current?.focus();
    ...
    try {
      ...
      const payload = {
        ...
        quantityUnit: form.quantityUnit !== undefined && form.quantityUnit !== null ? form.quantityUnit : undefined,
        ...
        accessory: {
          ...
          description: (form as any).accessoryDescription || undefined,
  ...
  return (
    <Drawer open={open} ModalProps={{ keepMounted: true }} anchor="right" onPaste={handlePaste} sx={{ zIndex: 9999 }} PaperProps={{ sx: { width: { xs: '100%', md: '85vw', xl: 1200 }, maxWidth: '100%', borderRadius: 0, display: 'flex', flexDirection: 'column' } }}>
      {/* Header */}
            ...
            <Box sx={{ flex: 1, overflowY: { xs: 'visible', md: 'auto' }, pr: { xs: 0, md: 1 } }}>
              ...
              <Card elevation={0} sx={{ borderRadius: 0, boxShadow: '0px 4px 20px rgba(0, 0, 0, 0.04)' }}>
                ...
                <CardContent sx={{ p: 3 }}>
                  <Stack spacing={3}>
                    <Box>
                      <Typography variant="caption" color="text.secondary" fontWeight={700} mb={1} display="block">Accessory Image</Typography>
              ...
              <Collapse in={showItemDetails}>
                <CardContent sx={{ p: 0 }}>
                  ...
                  <Stack spacing={0} divider={<Divider sx={{ borderStyle: 'dashed', borderColor: '#e2e8f0', mx: 4 }} />}>
                        ...
                        <Autocomplete 
                          ...
                          renderInput={(params) => (
                            <TextField 
                              ...
                              label={t('rdMaterial.category', 'Category')} 
                              required 
                              sx={{ 
                                '& .MuiOutlinedInput-root': { 
                                  bgcolor: errors.specification ? '#fef2f2' : '#f8fafc', 
                                  borderRadius: 1, 
                                  '& .MuiOutlinedInput-notchedOutline': {
                                    borderColor: errors.specification ? '#ef4444' : (shakeFields.specification ? '#ef4444' : undefined),
                                    borderWidth: (errors.specification || shakeFields.specification) ? '1.5px' : undefined,
                                  },
                    ...
                    {/* 4. Cost */}
                    <Box p={4}>
                      <Typography variant="subtitle2" sx={{ color: '#0f172a', textTransform: 'uppercase', letterSpacing: 1 }} fontWeight={800} mb={3}>{t('rdMaterial.cost_title', '4. Cost')}</Typography>
                      <Box display="grid" gridTemplateColumns={{ xs: '1fr', sm: '1fr 1fr' }} gap={2.5}>
...
export default AccessoryFormDrawer;
