...
interface YardageTableRowProps {
  item: Item;
  isSelected: boolean;
...
const YardageTableRow = React.memo<YardageTableRowProps>(({
  item,
  isSelected,
  onToggleSelect,
  filteredCols,
...
const YardageListPage: React.FC = () => {
  ...
  const [columnOrder, setColumnOrder] = useState<string[]>(() => {
    const defaultOrder = [
      'Select', 'Image', 'Name', 'Item Code', 'Structure', 'Composition', 'Function', 'GSM',
  ...
  const handleOpenDetails = useCallback((id: number) => {
    React.startTransition(() => navigate(`${BASE}/yardage/${id}`));
  }, [navigate]);
                      ...
                      <Box sx={{ flexGrow: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 1, flexWrap: 'wrap' }}>
                          ...
                          {visibleColumns['Qty'] !== false && (
                            <Chip 
                              label={`${item.quantity ?? 0} ${item.quantityUnit || 'yds'}`} 
                              size="small" 
