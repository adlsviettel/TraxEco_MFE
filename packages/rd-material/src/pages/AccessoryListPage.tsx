...
import {
  ...
  Menu, Divider, useTheme, useMediaQuery, Fab
} from '@mui/material';
...
const getStickyHeaderStyle = (colId: string, filteredCols: any[]) => {
  const stickyIds = ['Select', 'Image', 'Name', 'Item Code'];
...
const AccessoryTableRow = React.memo<AccessoryTableRowProps>(({
  item,
  isSelected,
  onToggleSelect,
  filteredCols,
      ...
      {filteredCols.map((col) => {
        ...
        switch (colId) {
          ...
          case 'Item Code':
            return (
              <TableCell key={colId} sx={{
                py: 1.5, fontSize: 13, fontFamily: 'monospace',
          ...
          case 'Surcharge':
            return (
              <TableCell key={colId} sx={{ py: 1.5, fontSize: 13, color: '#3f4945' }}>
                {[item.moqSurcharge ? `MOQ: ${item.moqSurcharge}` : '', item.mcqSurcharge ? `MCQ: ${item.mcqSurcharge}` : ''].filter(Boolean).join(' | ') || '—'}
              </TableCell>
            );
          case 'Leadtime':
            return (
...
const AccessoryListPage: React.FC = () => {
  ...
  const STORAGE_KEY = `rd-material-state-ACCESSORY`;
  const pageCode = 'rd_accessory';
  ...
  const [visibleColumns, setVisibleColumns] = useState<Record<string, boolean>>(() => {
    try {
      const saved = localStorage.getItem('rd-accessory-columns');
      if (saved) return JSON.parse(saved);
    } catch { /* ignore */ }
    return {};
  });
  ...
  useEffect(() => {
    localStorage.setItem('rd-accessory-columns', JSON.stringify(visibleColumns));
  }, [visibleColumns]);

  const [columnOrder, setColumnOrder] = useState<string[]>(() => {
    const defaultOrder = [
      'Select', 'Image', 'Name', 'Item Code', 'Supplier', 'Description', 'Size',
      'Color', 'Price', 'MOQ / MCQ', 'Surcharge', 'Leadtime', 'Origin', 'Location',
      'Holder', 'Qty', 'Created At', 'Remark', 'Actions'
    ];
    try {
      const saved = localStorage.getItem('rd-accessory-column-order');
      if (saved) {
  ...
  useEffect(() => {
    localStorage.setItem('rd-accessory-column-order', JSON.stringify(columnOrder));
  }, [columnOrder]);
  ...
  const columns = useMemo(() => [
    ...
    { id: 'MOQ / MCQ', label: 'MOQ / MCQ' },
    { id: 'Surcharge', label: 'Surcharge' },
    { id: 'Leadtime', label: 'Leadtime' },
    { id: 'Origin', label: 'Origin' },
    { id: 'Location', label: 'Location' },
    { id: 'Holder', label: 'Holder' },
    { id: 'Qty', label: 'Qty', isCenter: true },
  ...
  const handleOpenDetails = useCallback((id: number) => {
    React.startTransition(() => navigate(`${BASE}/accessory/${id}`));
  }, [navigate]);
  ...
  const handleExport = async () => {
    ...
    try {
      const data = await rdItemApi.getAll({ itemType: 'ACCESSORY', keyword, itemCode: itemCode.join(','), supplierName: supplierName.join(','), color: color.join(','), origin: origin.join(','), location: location.join(','), holder: holder.join(','), page: 0, size: 10000 });
      await exportRdItemsToExcel(data.content ?? [], 'ACCESSORY', t);
              ...
              items.map((item) => {
                ...
                return (
                  <SwipeableItem
                    ...
                    onSwipeRight={() => React.startTransition(() => navigate(`${BASE}/accessory/${item.id}`))}
                    rightActions={rightActions}
                  >
                    <Box 
                      onClick={() => React.startTransition(() => navigate(`${BASE}/accessory/${item.id}`))}
                      sx={{ 
                        p: 1.5,
                        display: 'flex',
                        gap: 1.5,
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.25, mt: 0.5 }}>
                          {item.accessory?.specification && (
                            <Typography variant="body2" sx={{ color: 'text.secondary', fontWeight: 500, fontSize: '0.75rem' }} noWrap>
                              <span style={{ color: '#9ca3af', marginRight: 4 }}>Spec:</span> <span style={{ color: '#475569' }}>{item.accessory.specification}</span>
                            </Typography>
                          )}
                        </Box>
