            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--bg-secondary, #f8fafc)', padding: '6px 12px', borderRadius: 8, border: '1px solid var(--border-color, #e2e8f0)' }}>
                ...
                <input
                  ...
                  style={{ border: '1px solid #cbd5e1', borderRadius: 6, padding: '4px 10px', fontSize: 13, fontWeight: 600, color: '#0f172a', background: '#fff' }}
                />
              </div>
            </div>
        ...
        <div className="card" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 400 }}>
          ...
          {loadingERP ? (
            <div style={{ textAlign: 'center', padding: '60px 20px', flex: 1 }}>
              ...
              <p style={{ color: '#64748b', margin: 0 }}>Querying Linked Server [192.168.70.115].AXDB Movement Journal...</p>
            </div>
            ...
            <>
              <div style={{ flex: 1, overflowY: 'auto', overflowX: 'auto' }}>
                <table className="table" style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0 }}>
                  ...
                  <tbody>
                    {pagedItems.map(item => (
                      <tr key={item.id} style={{ background: selectedIds.has(item.id) ? 'var(--primary-light, #e8f7ec)' : 'transparent', borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <td style={{ textAlign: 'center', padding: '10px' }}>
                          {item.statusPush === 'idle' && <span className="status-badge" style={{ background: '#f1f5f9', color: '#64748b' }}><Clock size={12} /> {t('common.pending', 'Pending')}</span>}
